...
void rOrdering_helper(jlcxx::ArrayRef<int> ord_data, const ring r)
{
  ...
  for (l = 0; r->order[l]; l++)
  {
    ...
    else if (ord == ringorder_wp || ord == ringorder_Wp || ord == ringorder_ws ||
             ord == ringorder_Ws || ord == ringorder_a || ord == ringorder_am ||
             ord == ringorder_M)
    {
...
void singular_define_rings(jlcxx::Module & Singular)
{
  ...
  Singular.method("rOrdering_helper",
                  &rOrdering_helper);    // inverse of rDefault_wvhdl_helper
...
// otherwise the number of variables for this letterplace ring
  ...
  Singular.method("rCoeffPtr", [](ring r) {
    return r->cf;
  });
  ...
  Singular.method("r_get_qideal", [](ring r) {
    return r->qideal;
  });
  Singular.method("is_qring", [](ring r) {
    return bool(r->qideal != NULL);
  });
  ...
  Singular.method("rBitmask", [](ip_sring * r) {
    return (unsigned int)r->bitmask;
  });
  ...
  Singular.method("p_Copy", [](spolyrec * p, ip_sring * r) {
    return p_Copy(p, r);
  });
  ...
  Singular.method("p_NSet", [](void * p, ip_sring * r) {
    return p_NSet(reinterpret_cast<snumber *>(p), r);
  });
  Singular.method("pLength", pLength);
  Singular.method("SetpNext", [](spolyrec * p, spolyrec * q) {
    p->next = q;
  });
  ...
  Singular.method("p_Init", [](ip_sring * r) {
    return p_Init(r);
  });
  ...
  Singular.method("pGetCoeff", [](spolyrec * p) {
    return pGetCoeff(p);
  });
  ...
  Singular.method("p_EqualPolys", [](spolyrec * p, spolyrec * q, ip_sring * r) {
  ...
  Singular.method("p_ExtGcd_internal", [](spolyrec * a, spolyrec * b, void * res,
                                          void * s, void * t, ip_sring * r) {
  ...
  Singular.method("p_PermPoly", [](poly p, int * perm, ring old_ring, ring new_ring,
                                   void * map_func_ptr, int * par_perm) {
  ...
  Singular.method("maFindPerm", [](ring src, jlcxx::ArrayRef<int> perm, ring dst,
    ...
    int * par_perm1 = NULL;
    if (rPar(src) != 0)
      par_perm1 = (int *)omAlloc0((rPar(src) + 1) * sizeof(int));
    maFindPerm(src->names, rVar(src), rParameter(src), rPar(src), dst->names, rVar(dst),
               rParameter(dst), rPar(dst), perm1, par_perm1, dst->cf->type);
  ...
  Singular.method("p_GetOrder", [](poly p, ring r) {
    long res;
    if (p != NULL)
    {
      res = p_GetOrder(p, r);
